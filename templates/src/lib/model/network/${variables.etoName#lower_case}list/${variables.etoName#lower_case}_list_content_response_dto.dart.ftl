...
class ${variables.etoName?cap_first}ListContentResponseDto {
  ...
  ${variables.etoName?cap_first}ListContentResponseDto(this.modificationCounter, this.id, <#list pojo.fields as field><#if field?has_next>this.${field.name}, <#else>this.${field.name} </#if></#list> );

...
import 'package:devon4ng_flutter_application_template/model/bloc/${variables.etoName?lower_case}insert/${variables.etoName?lower_case}_insert_bloc.dart';
...
class ${variables.etoName?cap_first}InsertLargeView extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _ScreenState();
}

class _ScreenState
    extends AbstractState<${variables.etoName?cap_first}InsertBloc, ${variables.etoName?cap_first}InsertLargeView> {
  final _formKey = GlobalKey<FormState>();
  <#list pojo.fields as field>
  <#if field?is_first><#else>final TextEditingController _${field.name}TextController() ;</#if>
  </#list>
  
  

  @override
  ${variables.etoName?cap_first}InsertBloc provideBloc() {
    return ${variables.etoName?cap_first}InsertBloc();
  }
    ...
    return Form(
        ...
        child: Column(children: <Widget>[
          ...
          <#list pojo.fields as field> <#if field?is_first><#else><#if field?has_next>_${field.name}TextController.text.isEmpty ||<#else>_${field.name}TextController.text.isEmpty</#if></#if></#list>{                 
              ...
              buildContext.read<${variables.etoName?cap_first}InsertBloc>().add(
                  ${variables.etoName?cap_first}InsertBlocEvent(
                    <#list pojo.fields as field><#if field?is_first><#else><#if field?has_next>_${field.name}TextController.text, <#else>_${field.name}TextController.text</#if></#if></#list>
                    ));
       }
    })
  ]));
